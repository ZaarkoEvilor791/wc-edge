...
# ADR 001 â€” Squad Array Ordering Contract
...
The squad array in `squadStore` is stored in a canonical sorted order: by position (GK â†’ DEF â†’ MID â†’ FWD), then by xP descending within each position group. This order is enforced at write-time inside `setSquad()` â€” callers pass raw arrays; normalization happens inside the setter.
...
`getXI()` must be stateless and fast â€” it is called on every render. The ordering approach makes it O(n) and pure. Alternatives considered:
...
- **Enforce ordering at every call site:** Was the prior (implicit) approach â€” spread the invariant across 4+ callers, making it easy to violate silently.
...
Do not suggest storing XI and bench as separate arrays in the store. The ordering approach was chosen deliberately to keep `getXI()` pure and avoid sync bugs. If you're seeing divergence between displayed XI and actual squad, the root cause is a mutation that bypasses `setSquad()` â€” fix that, don't add state.
